...
setwd("C:/Users/Anna/Documents/pancancer/CRC")
data <- read.csv("celltype_sample_counts_ImmuneEnrichrFiltered.csv", header = TRUE, row.names = 1, check.names = FALSE)
prop <- sweep(data, 2, colSums(data), "/")
prop <- prop + min(prop[prop>0])/2
clr_mat <- t(as.matrix(clr(t(prop))))

expr_counts <- read.csv("pseudobulk_malignant_immuneEnrichrFiltered.csv", header = TRUE, row.names = 1, check.names = FALSE)
expr_counts <- t(expr_counts)

ensg_ids <- sub("\\..*$", "", rownames(expr_counts))
symbols <- mapIds(
  org.Hs.eg.db,
  keys = ensg_ids,
  column = "SYMBOL",
...
new_rownames <- ifelse(is.na(symbols), ensg_ids, symbols)
rownames(expr_counts) <- new_rownames

metadata <- read.csv("sample_metadata_2.csv", row.names=1)
...
tcell_prop <- as.numeric(clr_mat["CD8 T cell", ])
...
hist(phathology_res[phathology_res$statistic <20,]$statistic, breaks = 100)
immune_genes<-phathology_res[is.na(phathology_res$statistic) == FALSE & phathology_res$statistic < 5,]$Gene.name

gene_stats <- read.csv("gene_stats_malignant_immuneFiltered_2.csv", check.names = FALSE, header = TRUE)
gene_stats$ensembl_clean <- sub("\\..*$", "", gene_stats$gene)
gene_stats$symbol <- mapIds(
  org.Hs.eg.db,
  keys = gene_stats$ensembl_clean,
  column = "SYMBOL",
  keytype = "ENSEMBL",
  multiVals = "first"
)
gene_stats$symbol <- ifelse(is.na(gene_stats$symbol), 
                            gene_stats$ensembl_clean, 
                            gene_stats$symbol)
...
banned_genes <- gene_stats[(gene_stats$pct_cells_expressed <= 0.3 |
                             ...
                             gene_stats$symbol %in% immune_genes,]$symbol
...
set.seed(42)
fit <- cv.glmnet(t(expr_norm), y, alpha = 0.5)
...
clinical_vars <- c('medical_condition', 'tumor_source', 'tumor_stage', 
                   'histological_type', 'microsatellite_status', 
                   'mismatch_repair_deficiency_status', 'treatment_response', 
                   'RECIST', 'platform')
...
ggsave("./CLR_nonzero/signature_by_clinical_vars.png", p_combined, width = 15, height = 10)
  

clinical_vars <- c('medical_condition', 'tumor_stage', 
...
for (var in clinical_vars) {
  if (var %in% colnames(metadata)) {
    ...
    temp_df <- temp_df[
      !is.na(temp_df$var_value) &
        temp_df$var_value != "" &
        tolower(as.character(temp_df$var_value)) != "nan",
    ]
    if (nrow(temp_df) > 0 && length(unique(temp_df$var_value)) > 1) {
      p <- ggplot(temp_df, aes(x = var_value, y = signature_score, fill = var_value)) +
        geom_boxplot(alpha = 0.7, outlier.size = 1.5) +
      ...
      if (length(unique(temp_df$var_value)) >= 2) {
        ...
        p <- p + annotate("text", 
                          x = 0.5, 
                          y = max(temp_df$signature_score, na.rm = TRUE) + 0.15,
                          label = paste("ANOVA p =", p_label),
                          size = 3.5,
                          hjust = 0)
      }
...
ggsave("./CLR_nonzero/signature_by_clinical_vars_2.png", p_combined, width = 12, height = 8)
...
print(p_cluster)
ggsave("./CLR_nonzero/signature_by_cluster.png", p_cluster, width = 6, height = 4)
...
plot_enrichment_results <- function(enrich_res, title = "", n_top = 15) {
  if (is.null(enrich_res) || nrow(enrich_res) == 0) {
    return(NULL)
  }
  
  top_terms <- enrich_res[1:min(n_top, nrow(enrich_res)), ]
  
  top_terms$neg_log_p <- -log10(top_terms$P.value)
  
  top_terms$gene_count <- sapply(strsplit(as.character(top_terms$Genes), ";"), length)
  
  top_terms <- top_terms[order(top_terms$neg_log_p), ]
  
  p <- ggplot(top_terms, aes(x = reorder(Term, neg_log_p), 
                             y = neg_log_p, 
                             fill = Database)) +
...
gene_stats_final <- gene_stats[gene_stats$symbol %in% gene_coef_final$feature, ]
colnames(gene_coef_final)[colnames(gene_coef_final) == "feature"] <- "gene"
combined_final <- merge(gene_coef_final, gene_stats_final, by.x = "gene", by.y = "symbol")
